...
if [ -f "$manualExplorerDastConfig" ]; then
  ...
  # Check if there is template file in root repository folder and upload to ASoC  
if [ -f "$templateFile" ]; then
    templateFildId=$(curl -k -s -X 'POST' "https://$serviceUrl/api/v4/FileUpload" -H 'accept:application/json' -H "Authorization:Bearer $asocToken" -H 'Content-Type:multipart/form-data' -F "uploadedFile=@$templateFile" | grep -oP '(?<="FileId":\ ")[^"]*');
    echo "$templateFile file exist. So it will be uploaded to ASoC and will be used to navigate in the URL target during tests. Manual Explorer file id is $templateFileId.";
else
    echo "Manual Explorer file not identified.";
fi
...
if [[ -n $appscanPresenceId ]]; then
    ...
    if [ -f $manualExplorerDastConfig ] && [ -f $loginDastConfig ]; then
        ...
        scanId=$(curl -k -s -X 'POST' "https://$serviceUrl/api/v4/Scans/Dast" -H 'accept:application/json' -H "Authorization:Bearer $asocToken" -H 'Content-Type:application/json' -d  '{"ScanConfiguration":{"Target":{"StartingUrl":"'"$urlTarget"'","ShouldScanBelowThisDirectory":false,"UseCaseSensitivePaths":false,"AdditionalDomains":[]},"Tests":{"TestPolicy":"Default.policy","TestOptimizationLevel":"Fast","TestLoginPages":false,"TestLoginPagesWithoutSessionIds":false,"TestLogoutPages":false},"Communication":{"ThreadNum":10,"ConnectionTimeout":null,"UseAutomaticTimeout":true,"MaxRequestsIn":10,"MaxRequestsTimeFrame":1000},"ApplicationElements":{"EnableAutomaticFormFill":true}},"TestOnly":false,"ExploreItems":[],"ScanTemplateId":"'"$templateFileId"'","PresenceId":"'"$appscanPresenceId"'","ScanName":"'"DAST $scanName $urlTarget"'","Locale":"en","AppId":"'"$appId"'","ClientType":"user-site","FullyAutomatic":false,"Execute":true,"Recurrence":{"Rule":null,"StartDate":null,"EndDate":null}}'| jq -r '. | {Id} | join(" ")');
